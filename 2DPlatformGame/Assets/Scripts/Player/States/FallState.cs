...
namespace Player.States
{
    ...
    {
        public FallState(Enum stateID) : base(stateID)
        {
        }

        public override void OnEnter(StateMachine stateMachine)
        {
            stateMachine.Animator.SetBool(PlayerVariable.IsFall, true);
            
            // TODO: 这里需要设置一下标记，表示进入了下降状态，然后在 FixedUpdate 中判断标记并执行
            stateMachine.GetComponent<PlayerEntry>().IsFallFlag = true;
        }
        ...
        {
            stateMachine.Animator.SetBool(PlayerVariable.IsFall, false);
        }
    }
}